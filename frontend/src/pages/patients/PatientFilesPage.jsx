...
export function PatientFilesPage() {
  const { patientId = "" } = useParams(); const { user } = useAuth();
  const [filtersOpen, setFiltersOpen] = useState(false);
  const files = usePatientFiles({ patientId, user });
  ...
  if (files.loading) return <main className={styles.page}><SectionState type="loading" message="Cargando archivos clínicos…" /></main>;
  ...
  return <main className={styles.page}>
    ...
    <aside className={styles.notice}><p>Para agregar archivos a la historia clínica de {patientName}, primero registrá la evolución clínica. Después podrás adjuntarlos desde su detalle.</p><Link to={`/evolutions/new?patientId=${patientId}`}><Icon name="plus" size={18} />Crear evolución y adjuntar</Link></aside>
